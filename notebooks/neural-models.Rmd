...
```{r}
temp <- m1_predict(X = valid_X, Y_actual = valid_Y, w_h = c1_weights$w_h, b_h = c1_weights$b_h, w_o = c1_weights$w_o, b_o = c1_weights$b_o, act.fn = swish, act.fn.derive = swish_d1)
...
```{r}
predAsOneHot <- matrix(nrow = length(unique(temp$obsId)), ncol = 36)
i <- 1
for (oId in unique(temp$obsId)) {
  data <- temp[temp$obsId == oId, ]
  predAsOneHot[i, ] <- c(data$pred_t1, data$pred_t0, data$y_t1, data$y_t0)
  i <- i + 1
}
...
nnet <- neuralnet::neuralnet(
  formula = formula(paste0(paste(paste0("X", 1:18), collapse = " + "), "~.")),
  data = predAsOneHot,
  hidden = 10,
