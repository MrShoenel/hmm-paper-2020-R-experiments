...
buildCommitChains <- function(data, idCol, parentIdCol, returnOnlyInitial = F) {
  commitIds <- unique(data[[idCol]])
  parentIds <- unique(data[[parentIdCol]])
  chains <- list()
  
  initialIds <- commitIds[!(data$ParentCommitSHA1s %in% commitIds)]
  initialCommits <- data[data[[idCol]] %in% initialIds, ]
  ...
  for (i in 1:nrow(data)) {
    commit <- data[i, ]
    dataByParentId[[commit$ParentCommitSHA1s]] <- commit
  }
  
  for (i in 1:nrow(initialCommits)) {
    parent <- initialCommits[i, ]
...
treeXChains <- buildCommitChains(treeXL, "commitId", "ParentCommitSHA1s", returnOnlyInitial = FALSE)
```
